...
def saveUploadedImage(uploadedImage):
    try:
        with open(os.path.join('static/uploads',uploadedImage.name),'wb') as f:
...
def deleteUploadedImage(uploadedImage):
    try:
        ...
        os.remove(os.path.join('static/uploads', uploadedImage.name))
        return True
...
if uploadedImage is not None:
    ...
    if saveUploadedImage(uploadedImage):
        ...
        # process image 
        result = runModel(os.path.join('static/uploads',uploadedImage.name), model)
        # delete Image
